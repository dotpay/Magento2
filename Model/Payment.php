...
class Payment extends \Magento\Payment\Model\Method\AbstractMethod implements ConfigProviderInterface {
    ...
    \Magento\Framework\Model\Context $context, \Magento\Framework\Registry $registry, \Magento\Framework\Api\ExtensionAttributesFactory $extensionFactory, \Magento\Framework\Api\AttributeValueFactory $customAttributeFactory, \Magento\Payment\Helper\Data $paymentData, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, \Magento\Payment\Model\Method\Logger $logger, \Magento\Framework\Module\ModuleListInterface $moduleList, \Magento\Framework\Stdlib\DateTime\TimezoneInterface $localeDate, \Magento\Directory\Model\CountryFactory $countryFactory, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\App\ResourceConnection $resource, \Magento\Framework\App\DeploymentConfig $deploymentConfig, array $data = array()
    ) {
        parent::__construct(
    ...
    public function getPaymentOneClickImageUrl() {
        $baseUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_STATIC);
        return $baseUrl . 'frontend/Magento/luma/en_US/Dotpay_Dotpay/img/oneclick.png';
    }
    ...
    private function createTableOneClick() {
        $sqlCreateTable = <<<END
            CREATE TABLE IF NOT EXISTS `{$this->_tablePrefix}{$this->_tableOneClick}` (
                ...
                `oneclick_card_mask` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
                `oneclick_card_brand` varchar(20) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
                `oneclick_card_hash` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL,
                `oneclick_card_id` varchar(128) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
END;
     ...
     * 
     */
    public function cardList($empty = false) {
        $not = ($empty) ? '' : 'NOT';
...
END;
     ...
     * 
     */
    public function cardGetDataByOneclickId($oneclickId) {
        $sql = <<<END
            SELECT *
            FROM {$this->_tablePrefix}{$this->_tableOneClick}
            WHERE
...
END;
     ...
     * 
     */
    public function cardDeleteForCustomer($customerId) {
        $sql = <<<END
            DELETE FROM {$this->_tablePrefix}{$this->_tableOneClick}
